...
const getNewTokens= async({authorization,session})=>{
    ...
    if (!authorization) {
       throw AppError.NotAuthorizedError('Please, provide a token in request authorization header')
    }
      ...
      const tokens = tokenService.generateToken({ uid: user._id,sid: newSession._id  });
